            ...
            for it in range(1, MAX_ITERATIONS + 1):
                ...
                try:
                    obs = await asyncio.to_thread(
                        perception.observe, query, hits, history, prior_goals, run_id, _emit, it
                    )
                except Exception as e:
                ...
                try:
                    out = await asyncio.to_thread(
                        decision.next_step, goal, attached, history, tools, _emit, run_id, it
                    )
                except Exception as e:
                ...
                # Tool-call stall detection: same tool+args for same goal N times → force done
                tool_sig = f"{out.tool_call.name}:{sorted(out.tool_call.arguments.items())}"
                tool_bucket = _stall_tracker.setdefault(f"tool:{goal.id}", [])
                tool_bucket.append(tool_sig)
                if len(tool_bucket) >= 2 and len(set(tool_bucket)) == 1:
                    _emit(f"[agent6] TOOL STALL on '{goal.id}': {out.tool_call.name} called {len(tool_bucket)}x identically — force-marking done")
                    prior_goals = [
                        Goal(
                            id=g.id, text=g.text,
                            done=True if g.id == goal.id else g.done,

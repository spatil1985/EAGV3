...
- To "remember" or "store" a fact, use save_memory — NOT create_calendar_event.
- create_calendar_event is only for scheduling a future event or reminder on a specific date.
- If the goal references "the calculated date", "the result from the previous step", or similar, look at the most recent [answer] entry in the recent history — that value IS the date to use. Do not reuse dates mentioned in tool results.
"""
...
def next_step(
    ...
    for h in history[-6:]:
        ...
        if role == "answer":
            ...
            descriptor = h.get("result_descriptor", "")[:200]
            art_id = h.get("artifact_id", "")
            content_snippet = ""
            if art_id and artifacts.exists(art_id):
                try:
                    raw = artifacts.get(art_id).decode("utf-8", errors="replace")
                    content_snippet = f"\n  Content: {raw[:800]}"
                except Exception:
                    pass
            history_text += f"[tool:{h.get('tool')}] {descriptor}{content_snippet}\n"
    ...
    attached_total = sum(len(b) for b in attached_bytes)
    _log(f"[decision.in] goal={goal.text[:80]} | history={len(history)} entries | attachment={attached_total} bytes")
